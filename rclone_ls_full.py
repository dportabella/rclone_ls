...
#   files=rc.lsjson(shlex.quote(path), '--copy-links', '--log-level ERROR')
   files=rc.lsjson(shlex.quote(path), '--log-level ERROR')
   ...
   if ModTime is None:
      ModTime=rc.lsjson(shlex.quote(path), '--log-level ERROR', '--stat')[0]['ModTime']
